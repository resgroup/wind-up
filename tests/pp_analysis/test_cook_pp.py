...
class TestPowerClipping:
    def test_clip_to_rated_caps(self, full_data_pp_df: pd.DataFrame) -> None:
        # Create some bins with mean power above rated by artificially inflating the means
        mask = (full_data_pp_df["bin_mid"] >= 14) & (full_data_pp_df["bin_mid"] <= 20)
        full_data_pp_df.loc[mask, "pw_mean_pre"] *= 1.1
        result = _cook_pp(
            full_data_pp_df,
            pre_or_post="pre",
            ws_bin_width=WS_BIN_WIDTH,
        ...
        for col in ["pw_mean_pre", "pw_at_mid_pre"]:
            assert (result[col] <= RATED_POWER).all()

    def test_no_clip_allows_values_above_rated(self, full_data_pp_df: pd.DataFrame) -> None:
        # Create some bins with mean power above rated by artificially inflating the means
        mask = (full_data_pp_df["bin_mid"] >= 14) & (full_data_pp_df["bin_mid"] <= 20)
        full_data_pp_df.loc[mask, "pw_mean_pre"] *= 1.1
        result = _cook_pp(
            full_data_pp_df, pre_or_post="pre", ws_bin_width=WS_BIN_WIDTH, rated_power=RATED_POWER, clip_to_rated=False
        )
        for col in ["pw_mean_pre", "pw_at_mid_pre"]:
            assert (result[col] >= 0).all()
            assert result[col].max() > RATED_POWER

    def test_power_clipped_to_zero_at_low_ws(self, full_data_pp_df: pd.DataFrame) -> None:
        # Create some bins with mean power above rated by artificially inflating the means
        mask = full_data_pp_df["bin_mid"] < 3
        full_data_pp_df.loc[mask, "pw_mean_pre"] = -0.1  # set some slightly negative to test clipping at low ws
        result = _cook_pp(
            full_data_pp_df,
            pre_or_post="pre",
            ws_bin_width=WS_BIN_WIDTH,
...
class TestSiteMeanPcGapFilling:
    ...
    def test_without_site_mean_invalid_bins_clip_at_max_measured(self, sparse_high_ws_pp_df: pd.DataFrame) -> None:
        ...
        for col in ["pw_mean_pre", "pw_at_mid_pre"]:
            high_ws_pw = result.loc[result["bin_mid"] > self.rated_wind_speed, col]
            assert (high_ws_pw == self.low_observed_power).all()

    @pytest.mark.parametrize("clip_to_rated", [True, False])
    def test_with_site_mean_invalid_bins_reach_rated_power(
        self,
        clip_to_rated: bool,  # noqa: FBT001
        sparse_high_ws_pp_df: pd.DataFrame,
    ) -> None:
        site_mean_pc = _make_site_mean_pc_df()
        if clip_to_rated:
            # Create some bins with mean power above rated by artificially inflating the means
            mask = (site_mean_pc["bin_mid"] >= 14) & (site_mean_pc["bin_mid"] <= 20)
            site_mean_pc.loc[mask, "pw_clipped"] *= 1.1
            assert site_mean_pc["pw_clipped"].max() > RATED_POWER
        result = _cook_pp(
            sparse_high_ws_pp_df,
            pre_or_post="pre",
            ws_bin_width=WS_BIN_WIDTH,
            rated_power=RATED_POWER,
            clip_to_rated=clip_to_rated,
            site_mean_pc_df=site_mean_pc,
        )
        # With site mean curve, bins at rated ws should reach ~1300 kW
        for col in ["pw_mean_pre", "pw_at_mid_pre"]:
            high_ws_pw = result.loc[result["bin_mid"] >= self.rated_wind_speed, col]
            assert pytest.approx(high_ws_pw.iloc[0], abs=1e5) == 1263.888889
            assert (high_ws_pw.iloc[1:] == RATED_POWER).all()
