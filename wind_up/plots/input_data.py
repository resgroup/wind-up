...
def _validate_data_within_exclusions(
    ...
    for exclusion in exclusions:
        mask = _series.index.get_level_values("TurbineName") == exclusion[0]
...
def plot_input_data_timeline(
    ...
    *,
    ...
    scada_data_column_for_power: str = "ActivePowerMean",
    scada_data_column_for_yaw_angle: str = "YawAngleMean",
) -> plt.Figure:
    """Plot timeline of input data with key milestones and data exclusions.
    ...
    :param height_ratios: height ratios for the two subplots
    ...
    fig, (ax_turbines, ax_wf) = plt.subplots(
        ncols=1, nrows=2, sharex=True, figsize=figsize, gridspec_kw={"height_ratios": list(height_ratios)}
    )

    turbines = [t.name for t in _wu_cfg.asset.wtgs]
    y_values = list(range(1, len(turbines) + 1))
    ...
    for y_value_count, t in enumerate(turbines):
        ...
        turbine_series_power = (_df.query(f"TurbineName == '{t}'").droplevel(level="TurbineName", axis=0).sort_index())[
            pwr_col
        ]
        _plot_data_coverage(
        ...
        turbine_series_yaw = (_df.query(f"TurbineName == '{t}'").droplevel(level="TurbineName", axis=0).sort_index())[
            yaw_col
        ]
        _plot_data_coverage(
    ...
    if _wu_cfg.toggle is None:
        key_dates_styles = [
            ...
            },
            {
                ...
                "left": _wu_cfg.analysis_first_dt_utc_start,
                "right": _wu_cfg.upgrade_first_dt_utc_start,
            },
            *key_dates_styles,
        ]
    else:
