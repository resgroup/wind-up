...
def run_wind_up_analysis(
    ...
    for test_wtg_counter, test_wtg in enumerate(wtgs_to_test):
        ...
        test_df = test_df.to_pandas().set_index(DataColumns.timestamp)
        wf_df = (
            windfarm_df.to_pandas()
            .astype({DataColumns.turbine_name: str})
            .set_index([DataColumns.turbine_name, DataColumns.timestamp])
        )
    ...
    *,
    ...
    for other_test_wtg in wind_up_cfg.test_wtgs:
        if other_test_wtg.name == test_turbine_name:
            continue
        pw_na_before = test_df.select(pl.col(DataColumns.active_power_mean).is_nan()).sum().item()
        other_test_df = windfarm_df.filter(pl.col(DataColumns.turbine_name) == other_test_wtg.name)
        timestamps_to_filter = other_test_df.filter(pl.col(pw_col).is_null() | pl.col(ws_col).is_null()).select(
            DataColumns.timestamp
        )
        if not timestamps_to_filter.is_empty():
            cols_to_filter = list(
                {
                    pw_col,
                    ws_col,
            ...
            for c in cols_to_filter:
                test_df = test_df.with_columns(
                    pl.when(pl.col(DataColumns.timestamp).is_in(timestamps_to_filter.to_series()))
                    .then(pl.lit(np.nan))
                    .otherwise(c)
                    .alias(c)
                )
        pw_na_after = test_df.select(pl.col(DataColumns.active_power_mean).is_nan()).sum().item()
        print_filter_stats(
